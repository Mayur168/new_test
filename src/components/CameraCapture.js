...
const CameraCapture = ({
  ...
  const sendImageToBackend = async (imageSrc) => {
    try {
      ...
      const blob = await fetch(imageSrc).then((res) => res.blob());
      console.log("Image size (KB):", blob.size / 1024);
      if (blob.size / 1024 > 4000) {
      ...
      for (let i = 1; i < 3; i++) {
        try {
          ...
          console.error(`Retry attempt ${i + 1} failed:`, {
            ...
            request: retryError.request ? {
              readyState: retryError.request.readyState,
              status: retryError.request.status,
            } : null,
          });
          if (i === 2) {
            handleError("Failed to process image after retries. Please try again.");
            throw retryError;
          }
