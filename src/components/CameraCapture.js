...
const CameraCapture = ({ closeCamera, onDataReceived }) => {
  ...
  const startCamera = async () => {
    try {
      const mediaStream = await navigator.mediaDevices.getUserMedia({
        video: { facingMode, width: { ideal: 1280 }, height: { ideal: 960 }, aspectRatio: 4 / 3 },
        audio: false,
      });
      setStream(mediaStream);
      if (videoRef.current) {
  ...
  const sendImageToBackend = async (imageSrc) => {
    try {
      ...
      setBackendImage(response.data.image); // Assuming the backend returns an image URL or base64
    } catch (error) {
      console.error("Error sending image to backend:", error);
    }
  };
  ...
  const submitEditedImage = async () => {
    if (canvasRef.current) {
      const editedImageSrc = canvasRef.current.toDataURL("image/jpeg");
      try {
        const blob = await fetch(editedImageSrc).then((res) => res.blob());
        const formData = new FormData();
        ...
        console.log("Edited Image Response:", response.data);
      } catch (error) {
        console.error("Error sending edited image to backend:", error);
      }
    }
  };

  return (
    <div className="camera-container" style={{ position: "fixed", top: 0, left: 0, width: "100%", height: "100%", zIndex: 9999, backgroundColor: "black" }}>
      {!backendImage ? (
        <>
          <div className="webcam-wrapper" style={{ width: "100%", height: "100%", display: "flex", justifyContent: "center", alignItems: "center", position: "relative" }}>
            <video ref={videoRef} autoPlay playsInline muted style={{ width: "100%", height: "100%", objectFit: "cover" }}></video>
            <div className="camera-switch-overlay" onClick={switchCamera} style={{ position: "absolute", top: "10px", right: "10px", color: "white", cursor: "pointer", zIndex: 1000 }}>
              <FontAwesomeIcon icon={faCameraRotate} size="2x" />
            </div>
          </div>
          <div className="capture-button-container" style={{ position: "absolute", bottom: "20px", left: "50%", transform: "translateX(-50%)" }}>
            <button
              className="capture-button"
              onClick={captureImage}
              style={{ padding: "10px 20px", fontSize: "16px", borderRadius: "50px", border: "none", backgroundColor: "#1c75c4", color: "white", cursor: "pointer" }}
            >
              <FontAwesomeIcon icon={faCamera} style={{ marginRight: "5px" }} />
              Capture Photo
            </button>
        ...
        <div className="edit-container" style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
          <canvas ref={canvasRef} style={{ maxWidth: "100%", border: "1px solid #ccc", marginBottom: "20px" }}></canvas>
          <button onClick={handleImageEdit} style={{ marginBottom: "10px", padding: "10px 20px", backgroundColor: "#1c75c4", color: "white", border: "none", borderRadius: "5px" }}>
            Edit Image
          </button>
          <button onClick={submitEditedImage} style={{ padding: "10px 20px", backgroundColor: "#1c75c4", color: "white", border: "none", borderRadius: "5px" }}>
            Submit Edited Image
          </button>
        </div>
      )}
