...
const CameraCapture = ({ closeCamera, onDataReceived }) => {
    ...
    const switchCamera = async () => {
        setError(null);
        setShowCamera(false); // Hide the current camera

        const newFacingMode = facingMode === "user" ? "environment" : "user";

        try {
           const nextCamera = availableCameras.find(device =>
                device.label.toLowerCase().includes(newFacingMode) || device.label.toLowerCase().includes(newFacingMode === "user" ? "front" : "back"));
